...
const elegirMetodoDePago=(plan)=> {
  ...
  //*13/9 => Ejercicio de clase.
  /* Objeto que controla el funcionamiento del programa
const PlanDeSuscripcion=()=> {
    this.edad = 0;
    this.plan = "";
    this.metodoPago = "";
  
    // Constructor para solicitar la edad del usuario
    this.validarEdad = function () {
      this.edad = parseInt(prompt("Por favor, ingresa tu edad:"));
    ...
    this.elegirPlan = function () {
      this.plan = prompt("Elige un plan: scaled, intermedio, elite").toLowerCase();
  
      if (this.plan === "scaled" || this.plan === "intermedio" || this.plan === "elite") {
        this.elegirMetodoDePago();
      } else {
        alert("Plan no válido. Por favor, elige uno de los planes disponibles.");
      }
    };
  
    // Método para elegir el método de pago
    this.elegirMetodoDePago = function () {
      this.metodoPago = prompt("Elige un método de pago: efectivo, credito, debito").toLowerCase();
      let costoMensual = this.calcularCostoMensual();
  
      if (this.metodoPago === "efectivo") {
        alert("El costo mensual del plan " + this.plan + " para una persona de " + this.edad + " años es: $" + costoMensual + " (Pago en efectivo).");
      } else if (this.metodoPago === "credito") {
        alert("El costo mensual del plan " + this.plan + " para una persona de " + this.edad + " años es: $" + costoMensual + " (Pago con tarjeta de crédito).");
      } else if (this.metodoPago === "debito") {
        alert("El costo mensual del plan " + this.plan + " para una persona de " + this.edad + " años es: $" + costoMensual + " (Pago con tarjeta de débito).");
      } else {
        alert("Método de pago no válido. Por favor, elige uno de los métodos de pago disponibles.");
      }
    };
  
    // Método para calcular el costo mensual según el plan
    this.calcularCostoMensual = function () {
      switch (this.plan) {
        case "scaled":
          return 10000;
        case "intermedio":
  ...
  // Crear una instancia del objeto y utilizarla para iniciar el programa
  let planSuscripcion = new PlanDeSuscripcion();
  planSuscripcion.validarEdad(); */
